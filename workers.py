...
from consts import glob_boards, RABBITMQ_USER, RABBITMQ_PASSWORD, RABBITMQ_HOST_PORT, RABBITMQ_VHOST
from WormChan import memeater, small_memeater

# Create the celery app and get the logger
celery = Celery('tasks', broker=f'amqp://{RABBITMQ_USER}:{RABBITMQ_PASSWORD}@{RABBITMQ_HOST_PORT}/{RABBITMQ_VHOST}')
celery_log = get_task_logger(__name__)

@celery.task
def eat_mem_task(boards, username):
    memeater([f"/{board}/" for board in
              boards if board in glob_boards], username) 
